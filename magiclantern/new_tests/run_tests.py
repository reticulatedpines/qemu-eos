...
def main():
    ...
    suite = ml_tester.test_suite.TestSuite(args.cams,
                                           args.rom_dir,
                                           args.tests,
                                           args.fail_early)
    ...
    try:
        suite.run_tests()
    except ml_tester.test.TestError:
        # some test failed
        pass
    except ml_tester.test_suite.TestSuiteError:
...
def parse_args():
    ...
    supported_cams = ""
    for c in sorted(list(ml_tester.cam.Cam.supported_cams)):
        supported_cams += c + " "
    ...
    test_group_names = ""
    for n in ml_tester.test_group_names:
        test_group_names += n + " "
    ...
    if not args.tests:
        args.tests = ml_tester.test_group_names
    else:
        args.tests = set(args.tests)
        unknown_test_groups = args.tests - ml_tester.test_group_names
        if unknown_test_groups:
            print("FAIL: unknown test group name(s): %s" % unknown_test_groups)
            sys.exit(-1)

    if not args.cams:
        args.cams = list(ml_tester.cams.Cam.supported_cams)
